...
/**
 * Introduction 组件中单击示例时执行
 * @param value 单击的 prompt 值
 */
function onIntroductionPrompt(value: string) {
  prompt.value = value
          ...
          try {
            ...
            updateChat(
              ...
              dataSources.value.length - 1,
              {
                ...
                // 增加使用量
                usage: {
                  promptTokens: data.detail?.usage?.prompt_tokens,
            ...
            if (data.detail?.usage?.total_tokens)
              authStore.reduceBalance(data.detail?.usage?.total_tokens)
    ...
    <footer :class="footerClass">
      <div class="w-full max-w-screen-xl m-auto">
        <div class="flex items-center justify-between space-x-2">
          ...
          <HoverButton v-if="!isMobile" :tooltip="usingContext ? t('chat.turnOnContext') : t('chat.turnOffContext')" @click="toggleUsingContext">
